...
# 指定数据文件夹和输出文件夹
original_data_folder = "/data/tanlinfeng/IHD/HCOCO"
data_folder = "/data/smb_shared/@Dataset/IHD_FR/HCOCO/rerun"
backup_data_folder = "/data/smb_shared/@Dataset/IHD_FR/HCOCO/foreground_images"
output_folder = "/data/smb_shared/@Dataset/IHD_FR/HCOCO/rerun_examples"
...
# 读取数据文件
with open("HCOCO_fMSE.csv", "r") as f:
    lines = f.readlines()
...
for i, r in enumerate(ranges):
    ...
    for j in range(num_points_per_range):
        ...
        try:
            name_parts = point_name.split('_')
            shutil.copy(os.path.join(data_folder, f"{point_name}_2.jpg"), os.path.join(output_folder, folder_name, f"{point_name}_fore.jpg"))
        except FileNotFoundError:
            shutil.copy(os.path.join(backup_data_folder, f"{point_name}_2.jpg"), os.path.join(output_folder, folder_name, f"{point_name}_2.jpg"))
