...
dataset_path = Path('/data/tanlinfeng/IHD/Hday2night')
output_path = Path('/data/smb_shared/@Dataset/IHD_FR/Hday2night/foreground_images')

def process_line(line):
    parts = line.split()
    name = parts[0].split('.')[0]
    composite_image_path = get_first_file(dataset_path/'composite_images', name+'.*')
    foreground_image_path = get_first_file(output_path, name+'.*')
...
with open('fore3.txt', 'r') as f:
    lines = f.readlines()
    
    with open('hd2n_fMSE.csv', mode='w', newline='') as table:
